...
USingleTCPClient::USingleTCPClient()
{

}

int USingleTCPClient::RunClient()
{
	int retval;
...
bool USingleTCPClient::SendData(int&retval, SOCKET & sock, char * buf, int length, int flags)
{
	// 데이터 보내기
...
bool USingleTCPClient::ReceiveData(int&retval, SOCKET & sock, char * buf, int length, int flags)
{
	// 데이터 받기
...
// 사용자 정의 데이터 수신 함수
int USingleTCPClient::recvn(SOCKET s, char* buf, int len, int flags)
{
    int received;
    char* ptr = buf;
    int left = len;
...
void USingleTCPClient::addAditionalText(char * inputBuf,const char* text)
{
...
void USingleTCPClient::printCurrentTime()
{
